...
all_tasks = [t for t in response.data if t['status'] != 'canceled']

# Filtrace na vybraný měsíc
tasks_in_month = []
workplaces_set = set()
for t in all_tasks:
...
# Optimalizovaná detekce kolizí
collisions = detect_collisions_in_month(tasks_in_month)  # ← tuto funkci musíš mít přidanou výše!
...
for t in tasks_in_month:
    ...
    # Priorita barev
    if t['status'] == 'done':
        color = "#34A853"  # zelená pro hotovo

    colliding = collisions.get(t['id'], [])
    if colliding:
        color = "#EA4335"          # červená pro kolizi
        text_color = '#FFFFFF'     # bílý text na červeném

    # Tooltip kolizí (max 5 + ...)
    coll_str = []
    ...
    collision_tooltip = "Kolize s: " + ", ".join(coll_str) if coll_str else ""
    ...
    plot_data.append({
        ...
        "Color": color,
        "Collision": collision_tooltip,
        "TextColor": text_color   # pro dynamické zbarvení textu
    })
...
if not plot_data:
    ...
    fig.update_traces(
        ...
        hovertemplate="<b>%{hovertext}</b><br>Od: %{x|%d.%m.%Y}<br>Do: %{x2|%d.%m.%Y}<br>%{customdata[0]}",
        customdata=df["Collision"].values
    )

    fig.update_xaxes(tickformat="%d", tickmode="linear", dtick=86400000.0,
