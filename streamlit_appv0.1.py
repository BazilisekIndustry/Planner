...
def yyyymmdd_to_ddmmyyyy(date_str):
    if not date_str:
        return ""
    return datetime.strptime(date_str, '%Y-%m-%d').strftime('%d.%m.%Y')  # S tečkou pro český vzhled

def validate_ddmmyyyy(date_str):
    if not date_str:
        return True  # Volitelné datum je OK
        ...
        users = {
            'credentials': {
                'usernames': {
                    'admin': {
                        'name': 'Administrátor',
                        'password': 'admin123'  # Plain text – balíček zahashuje automaticky
                    }
                }
            },
            'cookie': {
...
if st.session_state.get('authentication_status'):
    ...
    # ============================
        ...
        if not project_choices:
            ...
            if not tasks:
                ...
                st.markdown("### Servisní mazání úkolu (pouze pro testování)")
                if tasks:
                    # Výběr podle pořadí (unikátní)
                    ...
                    if st.checkbox("Potvrďte smazání tohoto úkolu (neodvolatelné!)"):
                        if st.button("SMAZAT ÚKOL"):
                            if delete_task(task_to_delete[0]):
                                ...
                                recalculate_from_task(task_to_delete[0])  # Jen pro jistotu, i když už neexistuje
                                st.rerun()
                            else:
    ...
    # ============================
    # OSTATNÍ ZÁLOŽKY (zatím prázdné)
    # ============================
        # ============================
    # 3. HMG MĚSÍČNÍ – GANTT DIAGRAM
    # ============================
        # ============================
    # 3. HMG MĚSÍČNÍ – Gantt s denním rozlišením (jako původní canvas)
    # ============================
       # ============================
...
if st.session_state.get('authentication_status'):
    st.sidebar.markdown("---")
    st.sidebar.caption("Streamlit Plánovač Horkých komor – 2026")