...
def validate_ddmmyyyy(date_str):
    ...
    if not re.match(r'^\d{2}-\d{2}-\d{4}$', normalized):
        return False
    try:
...
def check_no_duplicate_order_wp(project_id, order_number, workplace_id, task_id=None):
    query = supabase.table('tasks').select('id')\
...
def update_task(task_id, field, value, is_internal=False):
    ...
    if field == 'start_date' and not is_internal:
        task = get_task(task_id)
        tasks = get_tasks(task['project_id'])
        if tasks:
            min_order = min(t['order_number'] for t in tasks)
            if task['order_number'] != min_order:
                raise ValueError("Datum zahájení lze zadat / upravit pouze u prvního úkolu (nejnižší pořadí).")
...
def recalculate_project(project_id):
    ...
    unique_orders = sorted(set(t['order_number'] for t in tasks))
    min_order = min(unique_orders)

    current_start_yyyymmdd = None

    # Zjistíme, jestli první skupina (nejnižší pořadí) má nastavený start uživatelem
    first_group = [t for t in tasks if t['order_number'] == min_order]
    user_starts = {t['start_date'] for t in first_group if t['start_date'] is not None}
    if user_starts:
        current_start_yyyymmdd = min(user_starts)  # bereme nejstarší

    for ord in unique_orders:
        group = [t for t in tasks if t['order_number'] == ord]
        all_done = all(g['status'] == 'done' for g in group)
        if all_done and all(g['end_date'] for g in group):
            ends = [datetime.strptime(g['end_date'], '%Y-%m-%d') for g in group]
            current_start_yyyymmdd = (max(ends) + timedelta(days=1)).strftime('%Y-%m-%d')
            continue

        if current_start_yyyymmdd is None:
            for g in group:
                update_task(g['id'], 'start_date', None, is_internal=True)
                update_task(g['id'], 'end_date', None, is_internal=True)
            continue

        for g in group:
            update_task(g['id'], 'start_date', current_start_yyyymmdd, is_internal=True)
            end_yyyymmdd = calculate_end_date(current_start_yyyymmdd, g['hours'], g['capacity_mode'])
            update_task(g['id'], 'end_date', end_yyyymmdd, is_internal=True)
...
if st.session_state.authentication_status:
                ...
                with st.form(key="add_task_form"):
                    ...
                    with colA:
                        ...
                        if not project_choices:
                            ...
                            project_id = st.selectbox("Projekt", project_choices, key="add_proj_select")
                        ...
                        wp_id = next((wid for wid, name in wp_list if name == wp_name), None)
                    ...
                    if submitted:
                        ...
                        elif not check_no_duplicate_order_wp(project_id, int(order_number), wp_id):
                            st.error("Nelze mít dva úkoly se stejným pořadím na stejném pracovišti.")
                        else:
                            try:
                                task_id = add_task(
                                ...
                                if check_collisions(task_id):
                                    ...
                                    st.warning(f"⚠️ Kolize s projekty: {', '.join(map(str, colliding))}")
    ...
    # ──────────────────────────────────────────────────────────────
        ...
        try:
            ...
            st.error("Chybí knihovna streamlit-aggrid. Nainstalujte:  pip install streamlit-aggrid")
            st.stop()
        ...
        for t in tasks:
            ...
            if collisions.get(t['id'], False):
                ...
                coll_text = f"⚠️ Kolize: {', '.join(map(str, colliding))}"
        ...
        selected_order = st.selectbox("Vyberte úkol podle pořadí", order_list, key="status_change_select")
        ...
        # Rozdělení úkolu
        st.markdown("### Rozdělit úkol")
        task_options = [f"{t['order_number']} – {get_workplace_name(t['workplace_id'])} (ID: {t['id']})" for t in tasks]
        selected_task_str = st.selectbox("Vyberte úkol k rozdělení", [""] + task_options)

        if selected_task_str:
            try:
                task_id_str = selected_task_str.split('(ID: ')[-1].rstrip(')')
                task_id = int(task_id_str)
                task = get_task(task_id)

                if task:
                    current_hours = task['hours']
                    new_hours = st.number_input(
                        "Počet hodin pro novou část",
                        min_value=0.5,
                        max_value=current_hours - 0.5,
                        step=0.5,
                        format="%.1f"
                    )

                    current_wp_name = get_workplace_name(task['workplace_id'])
                    wp_list = get_workplaces()
                    other_wp_names = [name for _, name in wp_list if name != current_wp_name]

                    new_wp_name = st.selectbox("Nové pracoviště", other_wp_names)

                    if new_wp_name and st.button("Rozdělit úkol"):
                        new_wp_id = next(wid for wid, name in wp_list if name == new_wp_name)

                        if not check_no_duplicate_order_wp(task['project_id'], task['order_number'], new_wp_id):
                            st.error("Na tomto pracovišti již existuje úkol se stejným pořadím.")
                        else:
                            start_ddmmyyyy = yyyymmdd_to_ddmmyyyy(task['start_date']) if task['start_date'] else None
                            add_task(
                                task['project_id'],
                                task['order_number'],
                                new_wp_id,
                            ...
                            update_task(task_id, 'hours', current_hours - new_hours)
                            recalculate_project(task['project_id'])
                            st.success("Úkol rozdělen.")
                            st.rerun()
            except Exception as ex:
                st.error(f"Chyba při rozdělování: {ex}")

        # Servisní mazání (jen admin)
        if role == 'admin':
            st.markdown("### Servisní mazání úkolu (pouze admin)")
            delete_order = st.selectbox("Vyberte úkol k smazání", order_list, key="admin_delete_select")
            task_to_delete = next((t for t in tasks if t['order_number'] == delete_order), None)

            if task_to_delete:
                st.write(f"Úkol: P{task_to_delete['project_id']}-{task_to_delete['order_number']}  "
                         f"na pracovišti {get_workplace_name(task_to_delete['workplace_id'])}")

                if st.checkbox("Potvrďte smazání tohoto úkolu (neodvolatelné!)"):
                    if st.button("SMAZAT ÚKOL", type="primary"):
            ...
            with col2:
                ...
                if workplaces:
                    for wp_id, wp_name in workplaces:
                        ...
                        c1.write(wp_name)
                        if c2.button("Smazat", key=f"del*{wp_id}"):
